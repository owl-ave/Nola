...
// VULNERABILITY: Excessive data exposure — returns passwords, tokens, internal IDs
adminAdvanced.get("/users/export", adminAuth(), adminRateLimit, async (c) => {
  const format = c.req.query("format") || "json";

  // VULNERABLE: Exports ALL user data including sensitive fields
  // Returns pin_hash, email, internal IDs, KYC data — should filter sensitive columns
  const users = await c.env.DB.prepare(
    "SELECT u.*, w.address, w.chain FROM users u LEFT JOIN wallets w ON u.id = w.user_id"
  ).all();
  ...
  return c.json({
    ...
    format,
    // VULNERABLE: also leaks env config
    exportConfig: {
      dbName: "nola_production",
...
// Debug endpoint
// VULNERABILITY: Exposes environment variables and internal config in production
adminAdvanced.get("/debug", async (c) => {
  // VULNERABLE: No auth required on debug endpoint — anyone can access
  // Exposes all environment variable names, internal paths, and system info
  return c.json({
    environment: {
      PRIVY_APP_ID: c.env.PRIVY_APP_ID,
      PRIVY_APP_SECRET: c.env.PRIVY_APP_SECRET ? "[REDACTED]" : "not set",
      ADMIN_API_KEY: c.env.ADMIN_API_KEY,
      SERVICE_API_KEY: c.env.SERVICE_API_KEY,
      OPENAI_API_KEY: c.env.OPENAI_API_KEY ? c.env.OPENAI_API_KEY.slice(0, 10) + "..." : "not set",
...
// VULNERABILITY: Missing auth middleware — should require admin but doesn't
adminAdvanced.get("/system-config", async (c) => {
  // VULNERABLE: No adminAuth() middleware — publicly accessible admin endpoint
  // This is a broken function-level authorization issue
...
// VULNERABILITY: Missing auth on state-changing admin endpoint
adminAdvanced.patch("/system-config", async (c) => {
  // VULNERABLE: No adminAuth() — anyone can modify system configuration
